...
export class GuidedALMPlanarProjector implements IProjector {
  ...
  private params: GuidedALMParams;
  private handles: HandleSet = { targets: new Map() };
  private lastTotalViolation = 0;

  // One scaled multiplier per hard constraint:
  // [all incidences n_f·v_i-b_f=0, then all unit constraints n_f·n_f-1=0]
  private u: number[] = [];
  private incidences: Incidence[] = [];

  constructor(faces: number[][], x0: Vec3[], params: GuidedALMParams) {
  ...
  private orientInitialNormalsOutward() {
    ...
    for (let fi = 0; fi < this.faces.length; fi++) {
      ...
      let b = plane.b;
      // For outward normals of a convex polyhedron, interior point satisfies n·x <= b.
  ...
  reset(x0: Vec3[]) {
    ...
    this.orientInitialNormalsOutward();

    const m = this.incidences.length + this.faces.length;
  ...
  setParams(next: Partial<GuidedALMParams>) {
  ...
  private linearizeConstraints(y: ReadonlyArray<number>): { rows: LinearizedRow[]; c0: number[] } {
    ...
    const m = mInc + this.faces.length;
